...
void PagingAllocator::addProcessToMemory(std::shared_ptr<Process> process, int startMemoryAddress)
{
	std::string processName = process->getName();
	int numFramesNeeded = process->getNumberOfPages();
	int startFrame = startMemoryAddress / this->mem_per_frame;
	if (numFramesNeeded > this->freeFrameList.size()) {
		std::cout << "Not enough free frames to allocate process " << processName << std::endl;
		return;
	}
	for (int i = 0; i < numFramesNeeded; i++) {
		int frame = this->freeFrameList.front();
		this->freeFrameList.pop();
		this->frameMap[frame] = processName;
		int actualAddress = startFrame * this->mem_per_frame;
		std::cout << "Process " << processName << " allocated at memory address " << actualAddress << std::endl;
	...
	std::cout << "Process " << processName << " added to memory" << std::endl;
}

void PagingAllocator::deallocateProcessFromMemory(std::shared_ptr<Process> process)
{
	std::string processName = process->getName();
	std::vector<int> frameIndexes;
	...
	std::cout << "Process " << processName << " deallocated from memory." << std::endl;
}

int PagingAllocator::getTotalExternalFragmentation() const
{
	int totalFreeMemory = this->freeFrameList.size() * this->mem_per_frame;
	int totalUnusedMemory = this->max_mem - (this->frameMap.size() * this->mem_per_frame);
	int totalExternalFragmentation = totalFreeMemory + totalUnusedMemory;
	return totalExternalFragmentation;
}

void PagingAllocator::printMemory(int cpuNo, int quantumCycle)
{
	std::cout << "Memory for CPU " << cpuNo << " at quantum cycle " << quantumCycle << std::endl;
	for (int i = 0; i < 15; ++i) {
		std::cout << "-";
	}
	std::cout << std::endl;
	for (const auto& pair : this->frameMap) {
		std::cout << "Frame " << pair.first << ": " << pair.second << std::endl;
	}
	for (int i = 0; i < 15; ++i) {
		std::cout << "-";
	}
...
std::string PagingAllocator::getDateNow()
{
	time_t now = time(0);
	tm* ltm = localtime(&now);
	std::string dateNow = std::to_string(1 + ltm->tm_mon) + "/" + std::to_string(ltm->tm_mday) + "/" + std::to_string(1900 + ltm->tm_year) + " " + std::to_string(ltm->tm_hour) + ":" + std::to_string(ltm->tm_min) + ":" + std::to_string(ltm->tm_sec);
	return dateNow;
}
